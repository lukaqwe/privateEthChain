...
3. Enter the directory `cd Node` and run the bashes in the following order:
    ...
    `bash nodeStart <RPCport> <ListeningPort>` - for running the console, note: RPCport and ListeningPort arguments are mandatory, the arguments are included if 2 nodes on same machine is desired, if so the ports must be different.
